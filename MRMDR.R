...
FCMDR <- function(phes, snp.mat, K=2, cv=10, nperm=1000, sele.type='cvc', covrt=NULL, trim=T, test.type='ht2'){
  ...
  result <- FCMDR_cv(data[aa,], snp.mat[aa,], K, cv=10, ratio = NULL, snp.combs, sele.type ='cvc', test.type)
...
#####################################################################################################
#### --------------------------------- subfunction of FCMDR -------------------------------- ####
#####################################################################################################

FCMDR_cv <- function (data, snp.all, K, cv=10, ratio = NULL, snp.combs, sele.type = 'cvc', test.type) {
  if (is.null(ratio)){
  ...
  for(i in 1:cv){
    ...
    for(j in 1:ns){
      # pb_comb$tick()
      temp.result <- FCMDR_cells(train.ids, test.ids, snp.all[, snp.combs[, j]], ratio, data, test.type)
...
#####################################################################################################
#### ---------------------------------- subfunction of FCMDR_cv -------------------------------- ####
#####################################################################################################
FCMDR_cells <- function (train.ids, test.ids, snp.mat, ratio, data, test.type) {
...
permutation_test <- function(data, snp.all, K, nperm, cv, test.type){
  ...
  for (i in 1:nperm){
    ...
    for(j in 1:cv){
      ...
      train.ids <- cc[-test.ids]
      temp.result <- FCMDR_cells(train.ids, test.ids, snp.all, ratio, perm.phes, test.type)
